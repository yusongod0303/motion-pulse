...
- 키보드 입력은 이제 누르고 있는 상태가 아니라 키다운 순간의 이벤트로 판정한다.
- 모션 입력도 존에 새로 진입하는 순간을 입력 이벤트로 처리한다.

현재 판정:

- `Perfect`: 노트 시간과 입력 시간이 80ms 이내
- `Great`: 150ms 이내
- `Good`: 240ms 이내
...
- 카메라 스트림은 화면 전환으로 video 엘리먼트가 바뀌어도 다시 연결되도록 `MediaStream`을 ref에 보관한다.
- 설정 화면에는 카메라 권한/스트림 상태를 표시하는 진단 영역을 둔다.
...
- 첫 화면을 파일 업로드 중심에서 내장 곡 리스트 중심으로 변경했다.
- `Pulse Training`, `Seoul Neon`, `Arcade Sprint` 내장 차트 카탈로그를 추가했다.
- 데모 곡은 브라우저 Web Audio API로 간단한 신스 비트를 생성해 노트와 함께 들리도록 했다.
- 키보드 테스트 모드에서 첫 노트에 맞춰 `Q`를 입력했을 때 `Great`, 점수 증가, 콤보 증가가 확인되었다.
...
- 기본 템포와 노트 밀도가 빨라서 초반 난이도가 높다.
- 카메라 영역과 하단 UI가 겹치거나 화면 아래쪽이 잘려 보인다.

반영 내용:

- `Pulse Training`을 입문용 Lv.1로 낮추고 BPM과 노트 수를 줄였다.
- `Seoul Neon`, `Arcade Sprint`도 기존보다 완만한 간격으로 조정했다.
- 설정 화면의 카메라 스테이지 높이를 줄이고, 키맵을 한 줄 compact chip으로 바꿔 1440x900 기준 첫 화면에서 주요 조작이 보이게 했다.
- 카메라 비디오는 `object-fit: contain`을 사용해 프레임이 잘리지 않게 했다.
- 플레이 화면의 상단/하단 HUD는 카메라 스테이지 위에 겹치지 않고 별도 줄로 배치했다.

검증:

- `npm run build` 통과.
- Browser 플레이테스트에서 설정 화면, 캘리브레이션 화면, 플레이 화면을 1440x900 viewport로 캡처해 하단 조작과 판정선이 가려지지 않는지 확인했다.
